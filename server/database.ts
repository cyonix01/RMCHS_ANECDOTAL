...
export async function getAllCriticalReports(): Promise<CriticalReport[]> {
  ...
  try {
    ...
    return allData.map((row: any) => ({
      id: row.id,
      studentLrn: row.student_lrn,
      dateOfIncident: row.date_of_incident || row.incident_date || "",
      timeOfIncident: row.time_of_incident || row.incident_time || "",
      incidentDate: row.date_of_incident || row.incident_date || "",
      incidentTime: row.time_of_incident || row.incident_time || "",
      issue: row.issue || row.nature_of_incident || "",
      natureOfIncident: row.issue || row.nature_of_incident || "",
      description: row.description || "",
      actionTaken: row.action_taken || row.immediate_action_taken || "",
      immediateActionTaken: row.action_taken || row.immediate_action_taken || "",
      recommendation: row.recommendation || "",
      reportedBy: row.reported_by || "",
      dateReported: row.date_reported || "",
      lastUpdatedBy: row.last_updated_by || "",
      recordStatus: row.record_status || "On Going",
      closureDate: row.closure_date,
    })) as any;
  } catch (err) {
    ...
    if (data && data.length > 0) {
      const supabaseLogs: AuditLog[] = data.map((row: any) => {
        ...
        return {
          ...
          ipAddress: row.ip_address || row.ip || undefined,
